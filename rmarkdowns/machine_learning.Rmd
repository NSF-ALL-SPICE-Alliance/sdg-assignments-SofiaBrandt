...
```{r}
importance_df <- as.data.frame(rf_cleanfuel$importance)
```
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  theme_minimal() +
  labs(title = "Most Important Variables in Clean Fuel",
       subtitle = "Top 10",
       y = "SDG Indicator",
...
```{r}
pdp::partial(rf_cleanfuel, pred.var = "normalized_score_sdg6_sanita", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(rf_cleanfuel, pred.var = c("normalized_score_sdg6_sanita", "normalized_score_sdg3_u5mort"))
# Default PDP
plotPartial(pd)
```
