...
```{r, warning = FALSE}
goal_4_and_3_scatterplot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point()+
  theme_minimal()+
  scale_fill_viridis_d(option="magma")+
  labs(title = "Distributions of SDG goal 4 and goal 3 scores",
       x = "SDG goal 3",
       y = "SDG goal 4",
       fill = "Region")
 

ggplotly(goal_4_and_3_scatterplot)
```

```{r}
goal_4_and_3_scatterplot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, fill= regions_used_for_the_sdr)) +
  geom_point()+
  theme_minimal()+
  scale_fill_viridis_d(option="magma")+
  labs(title = "Distributions of SDG goal 3 and goal 4 scores",
       x = "SDG goal 3",
       y = "SDG goal 4",
       fill = "Region")
...
mytext <- paste(
    ...
    "Goal 9 Score: ", round(sdr_data_world_joined$goal_7_score, 2), 
    sep="") %>%
...
leaflet(sdr_data_world_joined) %>% 
  ...
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("YlGn", goal_9_score)(goal_9_score), label = mytext)
```
