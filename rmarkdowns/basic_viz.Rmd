...
```{r}
ggplot(data = sdr_data, aes(x= goal_1_score, y = goal_2_score, color = goal_7_score)) +
  geom_point()
```
```
interpretation: The lightest blue points show contries with high SDG scores 7. These contries also seem to also have high goal 1 scores and high/mid goal 2 scores. The dark blue points are relatively dispersed throghout the graph, and are genearlly around the 50-60 sore values for goal 2. 
...
```{r}
ggplot(MENA_data, aes(x= goal_4_score, y = country)) +
  geom_bar(stat = "identity") 
```
...
interpretation: For sdg goal 3, the oced region countries have a higher sdg score compared to the sub-saharan africa region contries. E. Europe & C. Asia, Mena and LAC regions also tend to have a higher sdg score for goal 3. 
